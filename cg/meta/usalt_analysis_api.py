...
class UsaltAnalysisAPI:
    ...
    def __init__(self, db: Store, hk_api: hk.HousekeeperAPI, scout_api: scoutapi.ScoutAPI,
                 tb_api: tb.UsaltTrailblazerAPI, lims_api: lims.LimsAPI, deliver_api: DeliverAPI,
        ...
        self.LOG = logger
        self.usalt_fastq_handler = fastq_handler
    ...
    def start(self, case_obj: models.Family, **kwargs):
        ...
        # skip Usalt evaluation of QC criteria if any sample is downsampled/external
    ...
    def config(self, case_obj: models.Family) -> dict:
        """Make the Usalt config. Meta data for the case is taken from the case object
        and converted to Usalt format via trailblazer.

        Args:
            case_obj (models.Family):

        Returns:
            dict: config_data (Usalt format)
        """
        # Fetch data for creating a Usalt config file
        data = self.build_config(case_obj)

        # Validate and reformat to Usalt config format
        ...
        for link in case_obj.links:
            ...
            if sample_data['analysis_type'] in ('tgs', 'wes'):
                if link.sample.capture_kit:
                    ...
                    usalt_capturekit = CAPTUREKIT_MAP.get(link.sample.capture_kit)
                    sample_data['capture_kit'] = usalt_capturekit or link.sample.capture_kit
                else:
    ...
    def get_latest_metadata(self, case_id: str) -> dict:
        ...
        usalt_config_raw = self._get_latest_raw_file(case_id=case_id, tag='usalt-config')
        ...
        if usalt_config_raw and qcmetrics_raw and sampleinfo_raw:
            try:
                trending = self.tb.get_trending(usalt_config_raw=usalt_config_raw,
