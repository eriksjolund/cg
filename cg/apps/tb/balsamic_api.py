...
import click
import ruamel.yaml
from trailblazer.balsamic import files, fastq, trending
...
from .balsamic_add import AddHandler

log = logging.getLogger(__name__)


class BalsamicTrailblazerAPI(Store, AddHandler, fastq.FastqHandler):
    """Interface to Trailblazer for `cg`."""

    parse_sampleinfo = staticmethod(files.parse_sampleinfo)

    def __init__(self, config: dict):
        super(BalsamicTrailblazerAPI, self).__init__(
            config['trailblazer']['database'],
            families_dir=config['trailblazer']['root'],
        )
        self.balsamic_cli = BalsamicCli(config['trailblazer']['script'])
        self.balsamic_config = config['trailblazer']['balsamic_config']

    def start(self, family_id: str, priority: str = 'normal', email: str = None,
              skip_evaluation: bool = False, start_with=None):
        """Start Balsamic."""
        email = email or environ_email()
        kwargs = dict(config=self.balsamic_config, family=family_id, priority=priority, email=email,
                      start_with=start_with)
        if skip_evaluation:
            kwargs['skip_evaluation'] = True
        self.balsamic_cli(**kwargs)
    ...
    def parse_qcmetrics(data: dict) -> dict:
        """Call internal Trailblazer Balsamic API."""
    ...
    def get_trending(self, balsamic_config_raw: str, qcmetrics_raw: str, sampleinfo_raw: dict) -> dict:
        return trending.parse_balsamic_analysis(balsamic_config_raw=balsamic_config_raw,
